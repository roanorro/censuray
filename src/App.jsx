...
const supabase = {
  ...
  async signUp(email, password, displayName) {
    ...
    if (!res.ok || data.error || data.code) {
      throw new Error(data.msg || data.error_description || data.error?.message || data.message || 'Error al registrar');
    }
  ...
  async signOut() {
    try { await fetch(`${SUPABASE_URL}/auth/v1/logout`, { method: 'POST', headers: supabase.headers() }); } catch(e) {}
...
const api = {
  async processAudio(file, config) {
    ...
    const res = await fetch(`${BACKEND_URL}/api/process`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${supabase._accessToken}` },
      body: formData,
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({ detail: 'Error procesando' }));
      throw new Error(err.detail || 'Error procesando audio');
    }
    return res.json();
  },

  async applyCensorship(jobId, detections, replacementSound, paddingMs) {
    const res = await fetch(`${BACKEND_URL}/api/apply-censorship`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        replacement_sound: replacementSound,
        padding_ms: paddingMs,
      }),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({ detail: 'Error aplicando censura' }));
      throw new Error(err.detail || 'Error aplicando censura');
    }
    return res.json();
  },
};
...
const P = {
  light: {
    ...
    accent:'#6D28D9', accentSoft:'#8B5CF6', accentMist:'rgba(109,40,217,0.06)',
    accentGlow:'rgba(109,40,217,0.12)',
    ok:'#059669', okBg:'#ECFDF5', warn:'#D97706', warnBg:'#FFFBEB',
    err:'#DC2626', errBg:'#FEF2F2',
    border:'#E7E5E4',
    shadow1:'0 1px 2px rgba(28,25,23,0.04)',
    shadow2:'0 4px 24px rgba(28,25,23,0.06),0 1px 3px rgba(28,25,23,0.04)',
    shadow3:'0 12px 48px rgba(109,40,217,0.08),0 4px 16px rgba(28,25,23,0.04)',
  ...
  dark: {
    ...
    accent:'#A78BFA', accentSoft:'#C4B5FD', accentMist:'rgba(167,139,250,0.08)',
    accentGlow:'rgba(167,139,250,0.15)',
    ok:'#34D399', okBg:'#052E16', warn:'#FBBF24', warnBg:'#2D2005',
    err:'#F87171', errBg:'#2D0808',
    border:'#292524',
    shadow1:'0 1px 2px rgba(0,0,0,0.2)',
    shadow2:'0 4px 24px rgba(0,0,0,0.25)',
    shadow3:'0 12px 48px rgba(167,139,250,0.1),0 4px 16px rgba(0,0,0,0.3)',
    toggleOff:'#44403C', toggleOn:'#A78BFA',
  },
};

const PLAN_META = { free:{name:'Free'}, pro:{name:'Pro'}, business:{name:'Business'} };
const DEFAULT_FEATURES = {
  max_minutes_per_day: { enabled: true, limit_value: 30 },
  max_file_size_mb: { enabled: true, limit_value: 100 },
  category_profanity: { enabled: true },
  category_names: { enabled: false },
  category_personal_data: { enabled: false },
...
const SOUNDS = [
  { id:'bleep', label:'Bleep', desc:'Tono 1kHz', icon:'◉', requiresPro:false },
...
// Components
function Toggle({on, onChange, c, disabled}) {
  return (
    <button onClick={disabled ? undefined : onChange} style={{
      ...
      background: on ? c.toggleOn : c.toggleOff, position:'relative', transition:'background .2s',
      flexShrink:0, opacity: disabled ? .4 : 1,
    }}>
      <span style={{width:18, height:18, borderRadius:9, background:'#FFF', position:'absolute', top:3, left: on ? 23 : 3, transition:'left .2s', boxShadow:'0 1px 3px rgba(0,0,0,.15)'}}/>
    </button>
  );
}

function PlanBadge({plan, c}) {
  return (
    <span style={{fontFamily:'var(--font-mono)', fontSize:9, padding:'3px 8px', borderRadius:4, background: plan === 'pro' ? c.accentMist : c.warnBg, color: plan === 'pro' ? c.accent : c.warn, letterSpacing:'.05em', textTransform:'uppercase', fontWeight:600}}>
      {PLAN_META[plan]?.name}
    </span>
  );
}

function LoaderBars({c}) {
  return (
    <div style={{display:'flex', alignItems:'center', gap:3, height:32}}>
      {[0,1,2,3,4].map(i => (
        <div key={i} style={{width:3, borderRadius:2, background:c.accent, animation:`barPulse 1s ease-in-out ${i*.12}s infinite`}}/>
      ))}
    </div>
  );
}

function Section({title, subtitle, children, c, delay=0}) {
  return (
    <div style={{background:c.surface, borderRadius:14, border:`1px solid ${c.border}`, boxShadow:c.shadow1, padding:24, marginBottom:12, animation:`enterUp .5s cubic-bezier(.16,1,.3,1) ${delay}s both`}}>
      {title && (
        <div style={{marginBottom: subtitle ? 4 : 16}}>
          <h3 style={{fontFamily:'var(--font-body)', fontSize:15, fontWeight:600}}>{title}</h3>
          {subtitle && <p style={{fontFamily:'var(--font-body)', fontSize:12, color:c.textFaint, fontWeight:300, marginTop:2, marginBottom:16}}>{subtitle}</p>}
        </div>
      )}
      {children}
    </div>
...
function KeywordTag({word, onRemove, c}) {
  return (
    <span style={{display:'inline-flex', alignItems:'center', gap:6, padding:'5px 10px 5px 12px', borderRadius:8, background:c.accentMist, border:`1px solid ${c.accent}22`, fontFamily:'var(--font-body)', fontSize:13, fontWeight:500, color:c.accent}}>
      {word}
...
function DetectionRow({det, onToggle, onPlay, playing, c}) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{display:'flex', alignItems:'center', gap:12, padding:'12px 16px', borderRadius:10, background: hov ? c.surfaceHov : 'transparent', borderBottom:`1px solid ${c.border}`, opacity: det.is_censored ? 1 : .55}}>
      <button onClick={onPlay} style={{minWidth:68, padding:'6px 10px', borderRadius:8, border:'none', background: playing ? c.accent : c.bgAlt, cursor:'pointer', display:'flex', alignItems:'center', gap:6}}>
        ...
        <span style={{fontFamily:'var(--font-mono)', fontSize:12, fontWeight:500, color: playing ? '#FFF' : c.text}}>{det.time_str}</span>
      </button>
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:3}}>
          <span style={{fontFamily:'var(--font-body)', fontSize:14, fontWeight:600, color: det.is_censored ? c.accent : c.textSoft, textDecoration: det.is_censored ? 'line-through' : 'none'}}>{det.word}</span>
          <span style={{fontFamily:'var(--font-mono)', fontSize:9, padding:'2px 6px', borderRadius:4, background:c.accentMist, color:c.accent, textTransform:'uppercase'}}>{det.category}</span>
        </div>
        <p style={{fontFamily:'var(--font-body)', fontSize:12, color:c.textFaint, fontWeight:300, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
          {det.context_before} <strong style={{color: det.is_censored ? c.accent : c.textSoft, fontWeight:600}}>{det.word}</strong> {det.context_after}
        </p>
      </div>
      <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2}}>
        <Toggle on={det.is_censored} onChange={onToggle} c={c}/>
        <span style={{fontFamily:'var(--font-mono)', fontSize:9, color:c.textFaint}}>{det.is_censored ? 'censurado' : 'original'}</span>
      </div>
    </div>
  );
}

function AuthInput({label, type, value, onChange, c, placeholder}) {
  return (
    <div style={{marginBottom:16}}>
      ...
      <input type={type} value={value} onChange={onChange} placeholder={placeholder} style={{width:'100%', padding:'12px 14px', borderRadius:10, border:`1.5px solid ${c.border}`, background:c.bgAlt, fontFamily:'var(--font-body)', fontSize:14, color:c.text, outline:'none', transition:'border .2s'}} onFocus={e => e.target.style.borderColor = c.accent} onBlur={e => e.target.style.borderColor = c.border}/>
    </div>
  );
}
...
export default function Censuray() {
  ...
  const logout = async () => { await supabase.signOut(); setUserMenu(false); setView('home'); resetJob(); };
  ...
  const resetJob = () => {
    setFile(null); setFileName(''); setFileSize(''); setDetections([]);
    setJobId(null); setProgress(0); setDownloadUrl(''); setDownloadFilename(''); setProcessError('');
  };
  ...
  const startProcessing = async () => {
    ...
    const interval = setInterval(() => {
      if (prog < 85) {
        ...
        const ph = [...phases].reverse().find(p => prog >= p.at);
        if (ph) setProcessingPhase(ph.text);
      }
    }, 400);

    try {
      const result = await api.processAudio(file, {
        categories: cats,
      ...
      clearInterval(interval);
      setProcessError(e.message || 'Error procesando el audio');
      setView('config');
    }
  };
  ...
  const applyAndDownload = async () => {
    ...
    setProcessError('');
    try {
      const result = await api.applyCensorship(jobId, detections, sound, padding);
      setDownloadUrl(result.download_url);
      setDownloadFilename(result.filename);
      setView('done');
    } catch(e) {
      setProcessError(e.message || 'Error generando el archivo');
    }
    setApplying(false);
  };
  ...
  const flagged = detections.filter(d => d.is_censored).length;
  const totalDet = detections.length;
  const filteredDet = detections.filter(d => {
    if (filterCat !== 'all' && d.category !== filterCat) return false;
    if (searchDet && !d.word.toLowerCase().includes(searchDet.toLowerCase())) return false;
    return true;
  });
    ...
    <div style={{minHeight:'100vh', background:c.bg, color:c.text, transition:'background .4s,color .4s'}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}::selection{background:${c.accent}22}
        ...
        .btn-fill{background:${c.accent};color:#FFF;padding:13px 28px}
        .btn-fill:hover{filter:brightness(1.1);box-shadow:${c.shadow3}}
        .btn-ghost{background:transparent;color:${c.textSoft};padding:13px 28px;border:1.5px solid ${c.border}}
        .btn-ghost:hover{border-color:${c.accent};color:${c.accent}}
        .btn-subtle{background:${c.accentMist};color:${c.accent};padding:10px 20px;font-size:13px}
        .btn-subtle:hover{background:${c.accentGlow}}
      ...
      <nav style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0 28px', height:56, borderBottom:`1px solid ${c.border}`, background: c.surface + (dark ? 'dd' : 'ee'), backdropFilter:'blur(16px)', position:'sticky', top:0, zIndex:100}}>
        <div onClick={() => { resetJob(); setView('home'); }} style={{cursor:'pointer', display:'flex', alignItems:'baseline', gap:6}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:10}}>
          ...
          {user ? (
            <div style={{position:'relative'}}>
              ...
              {userMenu && (
                <div style={{position:'absolute', right:0, top:44, width:240, background:c.surface, border:`1px solid ${c.border}`, borderRadius:12, boxShadow:c.shadow2, padding:8, zIndex:200, animation:'enterScale .15s ease'}}>
      ...
      {view === 'home' && (
        <div style={{maxWidth:640, margin:'0 auto', padding:'100px 24px 80px', animation:'enterUp .7s cubic-bezier(.16,1,.3,1)'}}>
          ...
          <h1 style={{fontFamily:'var(--font-display)', fontSize:'clamp(40px,7vw,64px)', fontWeight:400, fontStyle:'italic', lineHeight:1.05, letterSpacing:'-0.03em', marginBottom:20}}>
      ...
      {view === 'config' && (
        <div style={{maxWidth:660, margin:'0 auto', padding:'36px 24px 60px', animation:'enterUp .5s cubic-bezier(.16,1,.3,1)'}}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:12, padding:'12px 16px', borderRadius:10, background:c.surface, border:`1px solid ${c.border}`, marginBottom:20}}>
            <div style={{width:36, height:36, borderRadius:8, background:c.accentMist, display:'flex', alignItems:'center', justifyContent:'center', color:c.accent, fontFamily:'var(--font-mono)', fontSize:11, fontWeight:500}}>{fileName.split('.').pop()?.toUpperCase()}</div>
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontFamily:'var(--font-body)', fontSize:14, fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{fileName}</div>
              <div style={{fontFamily:'var(--font-mono)', fontSize:11, color:c.textFaint}}>{fileSize}</div>
            </div>
              ...
              return (
                <div key={cat.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 0', borderBottom:`1px solid ${c.border}`}}>
                  <div style={{display:'flex', alignItems:'flex-start', gap:12, flex:1}}>
                    <span style={{fontSize:18, marginTop:1}}>{cat.icon}</span>
                    <div>
                      <span style={{fontFamily:'var(--font-body)', fontSize:14, fontWeight:500, display:'flex', alignItems:'center', gap:6}}>{cat.label}{!allowed && <PlanBadge plan="pro" c={c}/>}</span>
                      <span style={{fontFamily:'var(--font-body)', fontSize:12, color:c.textFaint, fontWeight:300}}>{cat.desc}</span>
                    </div>
                  </div>
          ...
          <Section title="Sonido de reemplazo" c={c} delay={.25}>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8}}>
              {SOUNDS.map(s => (
                <button key={s.id} onClick={() => setSound(s.id)} style={{padding:'14px 16px', borderRadius:10, cursor:'pointer', background: sound === s.id ? c.accent : 'transparent', border:`1.5px solid ${sound === s.id ? c.accent : c.border}`, textAlign:'left', display:'flex', alignItems:'center', gap:12}}>
                  <span style={{fontFamily:'var(--font-mono)', fontSize:16, color: sound === s.id ? '#FFF' : c.accent}}>{s.icon}</span>
                  <div>
                    <span style={{fontFamily:'var(--font-body)', fontSize:13, fontWeight:600, color: sound === s.id ? '#FFF' : c.text, display:'block'}}>{s.label}</span>
                    <span style={{fontFamily:'var(--font-mono)', fontSize:10, color: sound === s.id ? 'rgba(255,255,255,.55)' : c.textFaint}}>{s.desc}</span>
                  </div>
                </button>
              ))}
            </div>
      ...
      {view === 'processing' && (
        <div style={{maxWidth:480, margin:'0 auto', padding:'80px 24px', animation:'enterScale .5s cubic-bezier(.16,1,.3,1)'}}>
          <div style={{background:c.surface, borderRadius:20, border:`1px solid ${c.border}`, boxShadow:c.shadow2, padding:'44px 32px', textAlign:'center'}}>
            ...
            <div style={{height:4, borderRadius:2, background:c.bgAlt, overflow:'hidden', marginBottom:12}}>
              <div style={{height:'100%', borderRadius:2, background:c.accent, width:`${progress}%`, transition:'width .3s'}}/>
            </div>
            <div style={{display:'flex', justifyContent:'space-between', fontFamily:'var(--font-mono)', fontSize:11, color:c.textFaint}}>
              <span>{fileName}</span>
      ...
      {view === 'review' && (
        <div style={{maxWidth:780, margin:'0 auto', padding:'36px 24px 60px', animation:'enterUp .5s cubic-bezier(.16,1,.3,1)'}}>
          ...
          {processError && (
            <div style={{padding:'12px 14px', borderRadius:10, background:c.errBg, border:`1px solid ${c.err}33`, marginBottom:16}}>
              <p style={{fontFamily:'var(--font-body)', fontSize:13, color:c.err}}>{processError}</p>
            </div>
          )}
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:1, background:c.border, borderRadius:12, overflow:'hidden', marginBottom:20}}>
            {[
              ...
              <div key={i} style={{padding:16, background:c.surface, textAlign:'center'}}>
                ...
                <span style={{fontFamily:'var(--font-body)', fontSize:22, fontWeight:700, color: s.a ? c.accent : c.text}}>{s.v}</span>
              </div>
            ))}
          </div>
                ...
                <div style={{display:'flex', gap:4, flexWrap:'wrap'}}>
                  {[{id:'all', label:'Todas'}, ...CATEGORIES].map(cat => (
                    <button key={cat.id} onClick={() => setFilterCat(cat.id)} style={{padding:'7px 12px', borderRadius:8, border:`1.5px solid ${filterCat === cat.id ? c.accent : c.border}`, background: filterCat === cat.id ? c.accentMist : 'transparent', color: filterCat === cat.id ? c.accent : c.textSoft, fontFamily:'var(--font-body)', fontSize:12, fontWeight:500, cursor:'pointer'}}>
                      {cat.icon && <span style={{marginRight:4}}>{cat.icon}</span>}{cat.label}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{background:c.surface, borderRadius:14, border:`1px solid ${c.border}`, overflow:'hidden'}}>
                <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:`1px solid ${c.border}`, background:c.bgAlt}}>
                  ...
                  <div style={{display:'flex', gap:8}}>
                    ...
                    <button onClick={() => setDetections(ds => ds.map(d => ({...d, is_censored: false})))} style={{fontFamily:'var(--font-mono)', fontSize:10, color:c.textFaint, background:'none', border:'none', cursor:'pointer'}}>Quitar todas</button>
                  </div>
                </div>
                <div style={{maxHeight:480, overflowY:'auto'}}>
                  {filteredDet.map(det => (
                    <DetectionRow key={det.id} det={det} onToggle={() => setDetections(ds => ds.map(d => d.id === det.id ? {...d, is_censored: !d.is_censored} : d))} onPlay={() => handlePlay(det.id)} playing={playingId === det.id} c={c}/>
                  ))}
                  {filteredDet.length === 0 && <div style={{padding:40, textAlign:'center'}}><p style={{fontFamily:'var(--font-body)', fontSize:14, color:c.textFaint}}>No se encontraron detecciones</p></div>}
                </div>
              </div>

              {playingId && (
                <div style={{marginTop:12, padding:'10px 16px', borderRadius:10, background:c.accentMist, border:`1px solid ${c.accent}22`, display:'flex', alignItems:'center', gap:12}}>
                  <div style={{width:8, height:8, borderRadius:'50%', background:c.accent, animation:'playPulse 1.5s ease-in-out infinite'}}/>
                  <span style={{fontFamily:'var(--font-body)', fontSize:13, color:c.accent, fontWeight:500}}>Reproduciendo contexto</span>
                </div>
              )}
            </>
          )}
