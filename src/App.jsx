...
const supabase = {
  ...
  async signUp(email, password, displayName) {
    ...
    // If we got a session immediately, user is logged in (email confirmation disabled)
  ...
  async signIn(email, password) {
    ...
    const data = await res.json();
    if (!res.ok || data.error || data.error_description || !data.access_token) {
      // Translate common errors to Spanish
      const rawError = data.error_description || data.msg || data.error?.message || data.message || "Credenciales inválidas";
      let friendly = rawError;
      if (/invalid.*credentials/i.test(rawError)) friendly = "Email o contraseña incorrectos";
      else if (/email.*not.*confirmed/i.test(rawError)) friendly = "Confirmá tu email antes de iniciar sesión. Revisá tu bandeja de entrada.";
      else if (/user.*not.*found/i.test(rawError)) friendly = "No existe una cuenta con ese email";
    ...
    try{
      ...
      if(authMode==="register"){
        ...
        if(data.needsConfirmation){
          setAuthSuccess("¡Cuenta creada! Revisá tu email para confirmar tu cuenta antes de iniciar sesión.");
          setAuthLoading(false);return;
        }
        setView("home");
              ...
              <>
                ...
                {authError&&(
                  <div style={{padding:"10px 12px",borderRadius:8,background:c.errBg,border:`1px solid ${c.err}33`,marginBottom:12,display:"flex",alignItems:"flex-start",gap:8}}>
                    <span style={{color:c.err,fontSize:14,lineHeight:1}}>⚠</span>
                    <p style={{fontFamily:"var(--font-body)",fontSize:13,color:c.err,lineHeight:1.4,fontWeight:500}}>{authError}</p>
                  </div>
                )}
